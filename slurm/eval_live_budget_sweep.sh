...
FRACTIONS=(0.25 0.30 0.35 0.40)
FRAC="${FRACTIONS[$SLURM_ARRAY_TASK_ID]}"
TAG=$(python3 -c "print('${FRAC}'.replace('.',''))")
...
python -m hbac.scripts.eval_compose_live \
  ...
  --output "results/compose_live_bf${TAG}_dpo_v2_sweep.json"

echo "Live budget sweep bf=${FRAC} done $(date)"
