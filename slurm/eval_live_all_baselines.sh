...
# 0=floor600 generous, 1=floor400 tight (includes ZEBRA in eval_compose_live)
FLOORS=(600 400)
FLOOR="${FLOORS[$SLURM_ARRAY_TASK_ID]}"
...
python -m hbac.scripts.eval_compose_live \
  ...
  --max-batches 50 \
  --output "results/compose_live_bf040_floor${FLOOR}_all_baselines_dpo_v2.json"

echo "All-baselines floor=${FLOOR} done $(date)"
