...
(4)~\textbf{Live conditions}: holdout-primary paired +0.45\pp for \texttt{hbac\_d18} vs.\ type-prior ($n{=}2000$; McNemar $p{\approx}0.004$; LCB-only); capability precondition met on LCB (Coder-Next-FP8 77--82\%) but not SWE (0\%).
...
Qwen2.5-7B/32B fail SWE; Qwen3-Coder-Next-FP8 clears LCB (77--82\%) but not SWE---live allocator study scoped to LCB+$\tau$+toolbench.
...
(4)~DPO v2 contaminated on LCB; holdout LoRA (LCB overlap=0) is the primary live agent.
(5)~L2 frozen ReAct limits joint hierarchical RL claims.
(6)~Local SWE harness is not Docker SWE-bench Verified grading.
...
Capability precondition~(i) is satisfied on LiveCodeBench with an open local model (Coder-Next-FP8); SWE remains out of scope pending Docker Verified grading.
...
\texttt{scripts/reproduce\_v3.sh} runs CPU analyses; GPU steps via \texttt{scripts/rivanna/submit\_holdout\_live.sh} and \texttt{submit\_capability\_2xa100\_vllm.sh}.
Canonical manifest: \texttt{results/canonical\_artifacts.json}.
Venue framing: NeurIPS Main (General) or Evaluations \& Datasets---contribution is the conditions characterization + open harness, not SWE SOTA.
