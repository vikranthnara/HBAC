...
LORA=$(ls -td checkpoints/llm_dpo/*_tau_v3b/ | head -1)/model
...
python -m hbac.scripts.eval_compose_live \
  ...
  --max-batches 50 \
  --output results/compose_live_tau_only_bf040_dpo_tau_v3b.json

echo "Tau v3b live eval done $(date)"
