#!/bin/bash
# Option 2: explicit CPU offload (bitsandbytes-safe spill to host RAM).
#SBATCH -J hbac_cap_offld
#SBATCH -p gpu
...
SHARD_DIR="${CAPABILITY_SHARD_DIR:-results/rivanna/capability_pilot_2xa100_offload_shards}"
OUT_JSON="${CAPABILITY_OUT_JSON:-results/rivanna/capability_pilot_2xa100_offload_uniform.json}"
ANALYSIS_JSON="${CAPABILITY_ANALYSIS_JSON:-results/capability_pilot_2xa100_offload_analysis.json}"

export HBAC_LLM_MODEL="${MODEL}"
export HBAC_LOAD_IN_4BIT=1
export HBAC_BNB_CPU_OFFLOAD=1
export HBAC_MAX_MEMORY_CPU="${HBAC_MAX_MEMORY_CPU:-128GiB}"
unset HBAC_BNB_GPU_ONLY HBAC_MAX_MEMORY_PER_GPU || true

echo "MODE=cpu_offload CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-unset}"
python - <<'PY'
import torch
...
echo "Capability cpu_offload pilot done $(date)"
