...
SHIFT="${SHIFT_FRACTION:-0.08}"
RESERVE="${SWE_MIN_RESERVE:-0.5}"
...
python -m hbac.scripts.eval_compose_live \
  ...
  --swe-min-reserve "${RESERVE}" \
  --budget-fraction 0.40 \
  --live-min-per-task 400 \
  --benchmarks "tau_bench,toolbench,mock,swe_bench" \
  --max-batches 50 \
  --output "results/compose_live_bf040_floor400_scarcity_refined_dpo_v2.json"

echo "D12 refined live eval done $(date) shift=${SHIFT} reserve=${RESERVE}"
