...
RUN_DIR=$(ls -td checkpoints/llm_dpo/*_capability/ | head -1)
LORA="${RUN_DIR}/model"
...
python -m hbac.scripts.eval_compose_live \
  ...
  --output results/compose_live_bf040_seed47_v2_dpo.json

echo "Live DPO v1 eval done $(date)"
